...
async function fetchUsers() {
    ...
    try { // error handinging using try la enna output varanum endu elluthirathu 
        const response = await fetch('https://randomuser.me/api/?results=5');
        const data = await response.json(); // await keyword entha function nadakum varaikum waiting
        const users = data.results;

        users.forEach(user => { // forEach enpathu loop
            // Create profile card
